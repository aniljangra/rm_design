	...
	function user_registration_form_field( $key, $args, $value = null ) {
		...
		switch ( $args['type'] ) {
			...
			case 'checkbox':
				...
				if ( isset( $options ) && array_filter( $options ) ) {
					...
					$field = '<div class="form-group form-check">
							<input id="agree" data-rules="' . esc_attr( $rules ) . '" data-id="' . esc_attr( $key ) . '" ' . implode( ' ', $custom_attributes ) . ' data-value="' . $value . '" type="' . esc_attr( $args['type'] ) . '" class="form-check-input fancy_input input-checkbox ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" value="1" ' . checked( $value, 1, false ) . ' /> <label for="agree" class="u ' . implode( ' ', $custom_attributes ) . '">'
			...
			case 'timepicker':
				...
				$field_des = '';
				if ( $args['description'] ) {
				$field_des = '<p class="message-right">' . $args['description'] . '</p>';
				}

				if ( empty( $extra_params ) ) {
					$field .= '<div class="form-group"> <input data-rules="' . esc_attr( $rules ) . '" data-id="' . esc_attr( $key ) . '" type="' . esc_attr( $args['type'] ) . '" class="form-control input-text input-' . esc_attr( $args['type'] ) . ' ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '"  value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' />'.$field_des.'</div>';
				} else {
					$field .= '<div class="form-group"><input data-rules="' . esc_attr( $rules ) . '" data-id="' . esc_attr( $key ) . '" type="' . esc_attr( $args['type'] ) . '" class="form-control input-text ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '"  value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' />'.$field_des.'</div>';
				}
				break;

			case 'color':
				$field .= '<div class="form-group"><input data-rules="' . esc_attr( $rules ) . '" data-id="' . esc_attr( $key ) . '" type="text" class=" form-control input-text input-color ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '"  value="' . esc_attr( $value ) . '" data-default-color="' . esc_attr( $args['default'] ) . '" ' . implode( ' ', $custom_attributes ) . ' /></div>';
				break;
		...
		if ( ! empty( $field ) ) {
			...
			if ( $args['label'] && 'checkbox' != $args['type'] ) {
				/*$field_html .= '<label for="' . esc_attr( $label_id ) . '" class="ur-label12">' . wp_kses(
					$args['label'],
					array(
						'a'    => array(
							'href'  => array(),
